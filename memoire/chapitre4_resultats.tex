...
Une perplexité de 1.37 signifie qu'en moyenne, le modèle hésite entre 1.37 choix de token suivant lors de la génération sur le domaine spécialisé. Pour comparaison, GPT-3 (175B paramètres) atteint une perplexité autour de 10 sur textes généraux \cite{brown2020gpt3}, tandis que le Mistral 7B base affiche 11.01 sur notre corpus DECP. La réduction de -87.5\% valide objectivement l'efficacité du fine-tuning sur domaine ciblé.
...
Le \textit{catastrophic forgetting} \cite{kirkpatrick2017overcoming} désigne la dégradation des capacités générales d'un modèle lors d'un fine-tuning spécialisé. Les travaux de French \cite{french1999catastrophic} et McCloskey \& Cohen \cite{mccloskey1989catastrophic} documentent des pertes de 10-40\% sur benchmarks généraux après fine-tuning sur corpus étroit.
...
\begin{itemize}
    ...
    \item \textbf{vs littérature GPT-3} : Brown et al. (2020) rapportent PPL \textasciitilde{}10-20 sur textes généraux \cite{brown2020gpt3}. La PPL de 1,37 sur domaine spécialisé suggère une adaptation supérieure, mais les corpus diffèrent (DECP vs web général)
